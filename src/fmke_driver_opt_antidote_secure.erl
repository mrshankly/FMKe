...
-define(MAP, antidote_secure_crdt_map_go).
-define(LWWREG, antidote_secure_crdt_register_lww).
-define(ORSET, antidote_secure_crdt_set_aw).

-define(SCRDT_KEY, <<40,226,49,96,86,129,89,27,141,148,35,205,3,14,205,94>>).
...
txn_read_object(Object, {Pid, TxnDetails}) ->
    {ok, [Value]} = antidotec_pb:read_values(Pid, [Object], TxnDetails, ?SCRDT_KEY),
    Value.
...
txn_read_objects(Objects, {Pid, TxnDetails}) ->
    {ok, Values} = antidotec_pb:read_values(Pid, Objects, TxnDetails, ?SCRDT_KEY),
    Values.
...
txn_update_object(ObjectUpdate, {Pid, TxnDetails}) ->
    ok = antidotec_pb:update_objects(Pid, [ObjectUpdate], TxnDetails, ?SCRDT_KEY).
...
txn_update_objects(ObjectUpdates, {Pid, TxnDetails}) ->
    ok = antidotec_pb:update_objects(Pid, ObjectUpdates, TxnDetails, ?SCRDT_KEY).
